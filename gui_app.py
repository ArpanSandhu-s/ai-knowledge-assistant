...
# # ─────────────────────────────────────────────────────────────────────────────
# # 15. GROUP CHAT ORCHESTRATION CORE LOOP
# #     Implements Azure AI Agent Design Pattern: Group Chat Orchestration
# #     Reference: https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/ai-agent-design-patterns
# #
# #     Flow:
# #       User Query → Intent Pre-fetch (weather/docs) → GroupChatOrchestrator
# #       → GroupChatManager (picks speaker) → Specialist Agent → Shared Transcript
# #       → Manager checks again → FINISH → Display to user
# # ─────────────────────────────────────────────────────────────────────────────
# from group_chat_engine import GroupChatOrchestrator

# # Agent display names and icons for the Streamlit UI
# AGENT_DISPLAY = {
#     "Data_Analyst":    ("🗂️", "Data Analyst Agent"),
...
#     with st.chat_message("assistant"):
#         with st.spinner("🔄 Group Chat Orchestrator routing to specialist agents…"):
#             final_text = ""
#             try:
#                 intent, extra = classify_intent(user_input)

#                 # ── PRE-FETCH PHASE ───────────────────────────────────────────
#                 # Gather any external data BEFORE passing to orchestrator
#                 # so agents receive rich context in the shared transcript

#                 doc_context  = ""
#                 weather_data = ""

#                 # Handle new_doc acknowledgement without spinning up full orchestration
#                 if intent == "new_doc":
#                     if st.session_state.indexed_filenames:
#                         names = ", ".join(st.session_state.indexed_filenames)
#                         final_text = (f"✅ **Indexed:** **{names}**\n\n"
...
#                 # Pre-fetch document chunks if doc intent detected
#                 if intent in ("summarize", "quiz", "key_points", "retrieve"):
#                     collection = st.session_state.active_collection
#                     if collection:
#                         st.caption("📂 *Retrieving document context for Data Analyst Agent…*")
#                         doc_context = query_active_collection(user_input, collection)
#                     else:
#                         final_text = ("⚠️ **No documents indexed yet.**\n\n"
#                                       "Please upload a PDF or text file using the sidebar first.")
#                         st.markdown(final_text)
#                         st.session_state.chat_history.append({"role": "assistant", "content": final_text})
#                         st.stop()

#                 # Pre-fetch weather data if weather intent detected
#                 if intent == "weather":
#                     st.caption(f"🌐 *Fetching live weather for Weather Agent…*")
#                     weather_data = get_weather(extra or "unknown")

#                 # ── ORCHESTRATION PHASE ───────────────────────────────────────
#                 # Hand off to Group Chat Orchestrator
#                 orchestrator = GroupChatOrchestrator(llm=chat_llm, max_rounds=4)
...
#                 # ── RENDER PHASE ──────────────────────────────────────────────
#                 # Display each agent turn with its badge
#                 st.markdown("---")
...
#                     # Skip system context injections — not for user display
#                     if sender in ("System_RAG_Context", "System_Weather_Feed", "User"):
#                         continue

#                     icon, label = AGENT_DISPLAY.get(sender, ("🤖", sender.replace("_", " ")))

#                     st.markdown(f"**{icon} {label}:**")
#                     st.markdown(turn["content"])
#                     st.markdown("---")

#                     # The last agent response becomes the persistent chat entry
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. SESSION STATE + PERSISTENT CHAT MEMORY
# ─────────────────────────────────────────────────────────────────────────────
CHAT_HISTORY_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "chat_memory.json"
)

def load_chat_history() -> list:
    """Load chat history from disk so it survives page refreshes."""
    try:
...
def save_chat_history(history: list):
    ...
    try:
        with open(CHAT_HISTORY_FILE, "w", encoding="utf-8") as f:
            json.dump(history[-50:], f, ensure_ascii=False, indent=2)  # keep last 50 turns
    except Exception:
        pass

for key, default in [("indexed_filenames", set()),
                     ("active_collection", None), ("index_status", "")]:
    if key not in st.session_state:
        st.session_state[key] = default

# Load persisted chat history on first run
if "chat_history" not in st.session_state:
        ...
        with st.spinner("🔄 Group Chat Orchestrator routing to specialist agents…"):
            ...
            try:
                ...
                for turn in transcript:
                    ...
                    # Skip system context injections — not for user display
                    if sender in ("System_RAG_Context", "System_Weather_Feed", "System_Conversation_Memory", "User", "Assistant"):
                        continue
